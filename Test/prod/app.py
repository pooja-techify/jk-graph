...
def request_password_reset():
    ...
    try:
        ...
        cursor.execute('SELECT username FROM prod.login WHERE username = %s', (username,))
...
def request_delete_user():
    ...
    try:
        ...
        cursor.execute('SELECT is_super FROM prod.login WHERE username = %s', (username,))
        result = cursor.fetchone()
        ...
        # Delete user
        cursor.execute('DELETE FROM prod.login WHERE username = %s', (username,))
        conn.commit()
...
def request_create_user():
    ...
    try:
        ...
        # Create new user
        cursor.execute('''
            INSERT INTO prod.login (username, password, permission_access)
            VALUES (%s, %s, %s)
...
def fetch_users():
    ...
    try:
        ...
        # Fetch all users
        cursor.execute('SELECT username, permission_access, is_super FROM prod.login')
        rows = cursor.fetchall()
